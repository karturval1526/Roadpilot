...
- Supabase usa una sesión anónima técnica únicamente durante el proceso OAuth; el usuario no crea una cuenta independiente ni usa correo/contraseña.
...
### Importante: activar Anonymous Sign-Ins
En Supabase ve a **Authentication → Sign In / Providers** y activa **Allow anonymous sign-ins**. Es necesario porque RoadPilot usa esa sesión técnica para iniciar el OAuth de Uber sin pedir un registro independiente.

Después ejecuta una vez `roadpilot-uber-only.sql` en SQL Editor.
...
## Servicios añadidos en esta versión
- Amazon Flex — servicio de bot de apoyo/monitoreo.
- DoorDash — servicio de bot de apoyo/monitoreo.
- Botones de contacto por WhatsApp.
- Botón de pago directo mediante Nequi:
  https://giros.nequi.com.co/l/CB3e1X5E

Los servicios se presentan como herramientas de monitoreo y asistencia. No incluyen aceptación automática de pedidos.


### Posicionamiento comercial de los servicios
Los servicios para Amazon Flex y DoorDash se presentan como herramientas para identificar y priorizar oportunidades con **mayor ingreso $$$ y menos millas**, usando filtros de valor, distancia y rendimiento. La aceptación de cada orden queda bajo control del conductor.


### Compra de planes
Los CTA visibles para clientes utilizan **Comprar plan**. El enlace de pago configurado se abre únicamente después de pulsar ese botón.
